...
def _assert_skill_md_roundtrips(content: bytes, *, expected_name: str) -> None:
    """Fail loudly (rather than ship a dead bundle) if the synthesized SKILL.md
    won't parse + validate through the receiver's strict loader on import."""
    with tempfile.TemporaryDirectory() as tmp:
        check_dir = Path(tmp) / expected_name
        check_dir.mkdir()
        (check_dir / "SKILL.md").write_bytes(content)
        load_skill(check_dir, source="synth-check", strict=True)
...
def _synthesize_skill_md(skill_dir: Path) -> bytes:
    ...
    # The description is the raw voice utterance — collapse all whitespace to a
    # single line so a stray newline or a lone "---" can't corrupt the SKILL.md
    # frontmatter the receiver parses; then clamp to the spec's 1024-char cap.
    description = " ".join(description.split())[:1024]
    frontmatter = {"name": name, "description": description, "compatibility": "protocol=skill"}
    yaml_text = yaml.safe_dump(
        frontmatter, sort_keys=False, allow_unicode=True, default_flow_style=False, width=4096
    ).rstrip()
    body = f"# {name}\n\n{description}\n\n_Generated by the KALI voice builder._\n"
    content = f"---\n{yaml_text}\n---\n\n{body}".encode("utf-8")
    _assert_skill_md_roundtrips(content, expected_name=name)
    return content
