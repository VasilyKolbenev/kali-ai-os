...
def package_skill(
    ...
    *,
    ...
    Args:
        skill_dir: Source skill dir (must contain SKILL.md).
    ...
    with tarfile.open(bundle_path, "w:gz") as tar:
        tar.add(skill_dir / "SKILL.md", arcname=f"{skill_dir.name}/SKILL.md")

        for subdir in ("references", "assets"):
            src = skill_dir / subdir
            if src.is_dir():
                tar.add(src, arcname=f"{skill_dir.name}/{subdir}")

        if include_scripts:
            scripts = skill_dir / "scripts"
            if scripts.is_dir():
                tar.add(scripts, arcname=f"{skill_dir.name}/scripts")
