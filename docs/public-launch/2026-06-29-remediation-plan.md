...
| **P1.2** | **Frozen-bundle live-verify (reel + loop)** | — | Audit #1 risk: PyAV native DLLs + reel font path unproven in `--onedir`. The in-flight backend build gives the first signal; then boot `kali-backend.exe` + hit `GET /skills/{name}/reel`, then a real create→works→share on the RTX machine. | Frozen smoke + screenshot of a real rendered reel |
...
| **P1.4** | **GPLv3 FFmpeg resolution** | M + legal | Audit high: 7 FFmpeg DLLs ship with NO license/NOTICE; avutil embeds GPLv3 + `--enable-gpl`/libx264 (desktop F5/torchcodec path, unrelated to the reel's LGPL libopenh264). Public-release blocker. Options: rebuild LGPL FFmpeg (no `--enable-gpl`/x264/x265), drop the torchcodec FFmpeg dep, or legal decision + bundle NOTICE. | License audit of rebuilt DLLs; F5 still synthesizes |
| **P1.5** | **Honest tethered-UGC framing** | S | Audit strategy: don't claim desktop-less receive in v1. Update landing copy (`docs/public-launch/index.html`) + the mobile connection UX so "tethered" is clear, not a dead-end. | Copy review; connection-screen guides pairing |
