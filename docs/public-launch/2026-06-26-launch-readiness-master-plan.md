...
>
> **Resolved (2026-06-27, task 3.2 + 3.8):** the flat-`packages` prototype client AND its tests were fully removed by the §4 rewrite (commit `85a472d`) — `kernel/catalog/client.py` now references only the §4 `skills`/`creators`/`installs` tables, and `tests/test_catalog_client.py` was rewritten to the §4 model. No flat-`packages` route/client/helper/table remains anywhere in `kernel/` (verified by grep). The `/catalog/*` routes in `main.py` already target the rewritten §4 `CatalogClient` (search/trending/social/moderation) or the `.kali-agent` package format — there is a single catalog source. The GitHub-curated `SkillsCatalog` path is intentionally kept and merges with Supabase in task **3.5**.
...
| 3.8 | ✅ **DONE (2026-06-27).** Decide the fate of the legacy `/catalog/*` routes + flat `packages` table (fold into the rewritten client or delete) to avoid two divergent catalogs at launch. **Outcome:** no divergent legacy path remained — the §4 rewrite (3.2, `85a472d`) had already deleted the flat-`packages` client + its tests, and the `/catalog/*` routes were already folded onto the §4 `CatalogClient` + `.kali-agent` format. Single catalog source confirmed (grep-verified); GitHub-curated `SkillsCatalog` kept for 3.5 merge; `local_search` preserved. | S | `kernel/main.py`, `kernel/catalog/*` |
