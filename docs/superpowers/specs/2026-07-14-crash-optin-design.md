...
**Шум поллинга vs диагностика (исправлено r3 по живым логам):** проб `/health` каждые 5с пока Python ЖИВ добавляет `/health`-строки в ТЕ ЖЕ файлы, которые мы собираем. Прежняя редакция этого абзаца утверждала, что шум оседает в `.out.log` и потому не вытесняет диагностику из `.err.log` — **это неверно:** `.out.log` на практике ПУСТ (0 байт), потому что Python логирует в stderr, а не в stdout. Реальная защита другая: берётся хвост из последних `CRASH_LOG_TAIL_LINES`(=400) строк каждого источника, а трейсбек краша — последнее, что пишется перед смертью, поэтому в хвост он попадает всегда; поллинг-шум вытесняет только более старый контекст. Приемлемо для v1; Rust-push по WS вместо поллинга — возможная будущая оптимизация (но это Rust-монитор+bus-событие+ws.rs — больше поверхности, против «ничего не ломать»).

**Порядок «err первым» остаётся верным — но по другой причине, чем говорила прежняя редакция:** не потому что шум уходит в `out`, а потому что сырой stderr ловит то, что проходит МИМО логгера (паники, C-уровень, падение до инициализации логирования).
...
2. Клик → `POST /crash/report` (тело `{reason?}`, опц.). Rust: `crash_paths()` → читает последние `CRASH_LOG_TAIL_LINES`(=400) строк из ТРЁХ источников (каждый свой хвост): `.err.log` (сырой stderr — ловит то, что мимо логгера), **`kali-backend.log` (собственный файловый лог Python — самая богатая диагностика и единственный источник при frozen/прямом запуске; добавлен r3 после прогона живых логов, где `.err.log` был 7 KB stale, а `kali-backend.log` — 967 KB)** и `.out.log` (обычно пуст); мета `version`(`CARGO_PKG_VERSION`), `os`/`arch`(`std::env::consts`), `ts`(`chrono::Utc::now()`), `reason` (кламп `CRASH_REASON_MAX_CHARS`=500 — он клиент-контролируем); всё через `redact()`; итог капается `CRASH_REPORT_MAX_BYTES`(=256 KB) — **режется по СТАРШЕМУ краю (сохраняем свежий хвост) с маркером `…(обрезано)`**; пишет `reports_dir/crash-<ts>.txt`; возвращает `{ path, text }`.
...
--- kali-backend.log (последние 400 строк, отредактировано) ---
<redacted tail>
...
Секции идут по убыванию диагностической ценности (err → app-log → out). Отсутствующий источник даёт строку `--- <имя>: логи не найдены ---`; если не найден НИ ОДИН — добавляется сводка «отчёт содержит только мету». Бюджет `CRASH_REPORT_MAX_BYTES` делится ПОРОВНУ между источниками (~87 KB на каждый при трёх), чтобы самый объёмный лог не вытеснил остальные.
