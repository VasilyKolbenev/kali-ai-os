...
// ── ShutdownControl: Completed / Timeout(retry) / Disconnected ────────────────
...
#[test]
fn shutdown_timeout_then_retry_success() {
    let (ctl, tx) = mk_shutdown();
    // ack не пришёл → Timeout; done остаётся false, receiver сохранён.
    assert_eq!(
        attempt_shutdown(&ctl, Duration::from_millis(50)),
        ShutdownResult::Timeout
    );
    assert!(!ctl.done.load(Ordering::SeqCst));
    // теперь supervisor подтвердил → retry успешен.
    tx.send(()).unwrap();
    assert_eq!(
        attempt_shutdown(&ctl, Duration::from_millis(500)),
...
#[test]
fn shutdown_disconnected_not_success() {
    let (ctl, tx) = mk_shutdown();
    drop(tx); // supervisor «умер» без ack
    assert_eq!(
        attempt_shutdown(&ctl, Duration::from_millis(50)),
        ShutdownResult::Disconnected
    );
    assert!(!ctl.done.load(Ordering::SeqCst), "disconnected ≠ success");
}
