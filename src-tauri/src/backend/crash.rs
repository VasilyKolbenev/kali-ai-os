...
///
/// **Цена правки — измерена, не «гипотетика».** `/` разрывает руну, поэтому
/// standard-base64 секрет со слэшем посередине теперь проходит МИМО, если ни
/// один сегмент не дотянул до 40. Замерено:
/// - 61-символьный секрет со `/` на позиции 32 (сегменты 32 и 28) проходит
///   **полностью незамаскированным**; тот же секрет без `/` — маскируется, а со
///   `/` на позиции 45 — маскируется (левый сегмент ≥40);
/// - на 100k случайных 64-символьных standard-base64: **63.6%** содержат хотя бы
///   один `/`, и у **26.7%** не остаётся просвета ≥40 → это и есть доля
///   context-free standard-base64, которую catch-all перестал ловить (примерно
///   четверть, не треть).
///
/// **Почему размен всё равно верный:** ни один секрет в стеке KALI не является
/// голой standard-base64 руной — control-plane токен это 64 hex (RE_HEX_RUN),
/// JWT это base64url с `-`/`_` (RE_JWT), ключи OpenAI/Anthropic/Google
/// префиксованы (RE_KEY_SK/RE_KEY_GOOGLE), Basic-креды короче 40 (их берёт
/// RE_AUTHORIZATION). То есть 26.7% теряются на классе, которого в стеке нет; и
/// бесконтекстными секреты в логах почти не бывают — рядом почти всегда есть
/// `=`/`Bearer`/`Authorization`, которые ловят свои паттерны. Против этого —
/// ГАРАНТИРОВАННЫЙ, рутинный, убивающий диагностику over-mask (см. примеры
/// выше). Финальный предохранитель — человек читает .txt перед отправкой.
///
/// Если в стек когда-нибудь придёт голый standard-base64 секрет — этот размен
/// нужно пересмотреть (вернуть `/` + отдельно исключить пути/URL).
///
...
fn tail_lines(path: &Path, n: usize) -> Option<String> {
    ...
    //
    // `lines.len() > 1`, НЕ `!is_empty()`: если в окне ровно одна строка (лог
    // >1 MB без единого перевода строки — например, один гигантский JSON-дамп),
    // выбросив её мы отдали бы ПУСТУЮ секцию. Лучше показать фрагмент: он всё
    // равно проходит через redact(), а пустая секция — потеря диагностики.
