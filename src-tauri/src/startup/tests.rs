...
// ── 20. KALI_BOOT_DELAY_MS: debug-only тест-рычаг задержки boot ───────────────
// A3 acceptance (owner): рычаг задерживает ТОЛЬКО признание готовности, без
// sleep (неблокирующая проверка дедлайна), не маскирует Foreign/реальные ошибки
// и не убивает живой child.
