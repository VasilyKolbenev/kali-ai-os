...
import { Check, Loader2, Search, Sparkles, Store, X } from "lucide-react";
...
import { CuratedStore } from "./CuratedStore";
...
type ViewId = "curated" | "advanced";

const COMMUNITY_SOURCE_ID = "kali";

export function AgentStore() {
  const [view, setView] = useState<ViewId>("curated");
  ...
  // Lazy community fetch on first visit to the chip
  useEffect(() => {
    if (category !== "community" || communityLoadedRef.current) return;
  ...
  const curatedEntries = useMemo(
    () => CURATED.filter((e) => e.kind !== "agent" || agentNames.size === 0 || agentNames.has(e.agentName!)),
    [agentNames],
  );
    ...
    <div className="w-full h-full p-8 overflow-auto">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-baseline gap-3 mb-6">
          <Store className="w-5 h-5 text-[var(--j-cyan)]" />
        ...
        <div className="mt-8 text-center">
          <button
            onClick={() => setView(view === "curated" ? "advanced" : "curated")}
            className="text-xs text-white/30 hover:text-white/60 transition underline-offset-4 hover:underline"
          >
            {view === "curated" ? "Для продвинутых: источники и публикация ›" : "‹ Вернуться в витрину"}
          </button>
        </div>
